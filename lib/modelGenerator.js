...
function getUI(model, attrName, paramId, labels) {
    ...
    if(!uiCache[attrName][paramId]) {
        const newUI = generateUI(getParameter(model, attrName, paramId), labels[attrName].parameters[paramId]);
...
function getParameterData(model, fullData, selectedViewId, viewIdx, attrName, parameterId, labels) {
    const ui = getUI(model, attrName, parameterId, labels),
...
export default function generateDataModel(model, fullData, selectedViewId, viewIdx, labels) {
    var propertyList = [],
    ...
    viewAttrs.forEach( (attrName) => {
        ...
        model.definitions[attrName].parameters.forEach( (paramAttr, idx) => {
            if (Array.isArray(paramAttr)) { //OR prop
                ...
                const prop = getParameterData(model, fullData, selectedViewId, viewIdx, attrName, paramAttr.id, labels);
                prop.viewData = viewData;
                if (paramAttr.show) {
